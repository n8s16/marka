...
- **Mark as paid** (bottom sheet from Bills) — pre-filled amount (editable), wallet picker, paid date, period selector (defaults to bill's due-month, user-editable), optional note.
...
- **Add / edit bill** — name, expected amount, frequency (monthly / quarterly / yearly / custom — custom takes an interval in months), due day, first due month (anchors quarterly / yearly / custom cadences; pre-filled to the current month), default payment source, reminder offset (days before) and reminder time (time of day), auto-forecast toggle.
